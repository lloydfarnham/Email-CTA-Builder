...
function Form() {
  ...
  const handleLabelChange = (event) => {setLabel(event.target.value.toUpperCase().replace("'", "’"))}
  ...
  const handleEnableImage = () => {setEnableImage((prevState) => !prevState)}
  ...
  const generateOutputCode = () => {
    ...
    <tr>
      <td align="center" style="margin: 0; padding: 4px;">
        <div>
        <!--[if mso]>
        ${enableImage ? 
